...
def get_orbit_files(orbit_type: OrbitType) -> list[str]:
    ...
    """
    prefix = "AUX_POEORB" if orbit_type == OrbitType.precise else "AUX_RESORB"

    orbit_files = list_public_bucket(ASF_BUCKET_NAME, prefix=prefix)

    logger.info(f"Found {len(orbit_files)} {orbit_type.name} orbit files")
    return orbit_files
